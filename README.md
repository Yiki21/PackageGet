...
`1.2.2` is an unsigned cross-platform release. This patch release includes Linux native packages, glibc/musl portable archives, AppImages, Windows x86_64 portable/setup packages, and macOS arm64/x86_64 app archives and DMGs. Verify every download against the matching `SHA256SUMS`; Windows SmartScreen and macOS Gatekeeper may warn because signing and notarization are intentionally outside the 1.0 policy. See [RELEASE_NOTES.md](RELEASE_NOTES.md) for platform limitations and compatibility notes.
