...
fn manager_config(config: &Config) -> ManagerConfig {
    let id = PackageManagerType::Flatpak.manager_id();
    config
        .manager(&id)
        .cloned()
...
mod tests {
    ...
    use super::*;
    #[test]
    fn config_preserves_custom_flatpak_path() {
        let config = Config {
            managers: vec![
                ManagerConfig::new(PackageManagerType::Flatpak.manager_id())
    ...
    #[test]
    fn direct_model_conversion_preserves_flatpak_metadata() {
