...
use updater_managers::FlatpakManager as DirectFlatpakManager;
...
async fn run_package_with_progress(
    ...
    let target = PackageTarget::new(PackageManagerType::Flatpak.manager_id(), package_name);
    DirectFlatpakManager::new()
...
fn manager_config(config: &Config) -> ManagerConfig {
    let manager_config = ManagerConfig::new(PackageManagerType::Flatpak.manager_id());
    if let Some(path) = config.get_package_path(PackageManagerType::Flatpak) {
...
mod tests {
    ...
    #[test]
    fn legacy_config_bridge_preserves_custom_flatpak_path() {
        let config = Config {
            app_managers: vec![PackageManagerConfig {
                manager_type: PackageManagerType::Flatpak,
                custom_path: Some("/custom/flatpak".to_owned()),
            }],
            ..Config::default()
        };

        let converted = manager_config(&config);
        assert_eq!(converted.id, PackageManagerType::Flatpak.manager_id());
        assert_eq!(
            converted.executable(),
            Some(std::path::Path::new("/custom/flatpak"))
        );
    }

    #[test]
    fn legacy_model_bridge_preserves_flatpak_metadata() {
        let id = ManagerId::parse("builtin:flatpak").expect("valid Flatpak ID");
        let mut package = ApiPackageInfo::new(id.clone(), "org.example.App", "1.2 (stable)");
        package.description = Some("Example application".to_owned());
        package.size = Some(42);
        package.homepage = Some("https://example.org/".to_owned());

        let converted = convert_package_info(package);
        assert_eq!(converted.name, "org.example.App");
        assert_eq!(converted.version, "1.2 (stable)");
        assert_eq!(converted.source, PackageManagerType::Flatpak);
        assert_eq!(
            converted.description.as_deref(),
            Some("Example application")
        );
        assert_eq!(converted.size, Some(42));
        assert_eq!(converted.homepage.as_deref(), Some("https://example.org/"));

        let update = ApiPackageUpdate::new(PackageTarget::new(id, "org.example.App"), "1.1", "1.2");
        let converted = convert_package_update(update);
        assert_eq!(converted.name, "org.example.App");
        assert_eq!(converted.current_version, "1.1");
        assert_eq!(converted.new_version, "1.2");
    }
