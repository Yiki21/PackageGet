...
fn manager_config(config: &Config) -> ManagerConfig {
    let id = PackageManagerType::Apt.manager_id();
    config
        .manager(&id)
        .cloned()
...
mod tests {
    ...
    use super::*;
    #[test]
    fn config_preserves_custom_apt_path() {
        let config = Config {
            managers: vec![
                ManagerConfig::new(PackageManagerType::Apt.manager_id())
                    .with_executable("/custom/apt"),
            ],
            ..Config::default()
        };
    ...
    #[test]
    fn direct_model_conversion_preserves_apt_metadata() {
