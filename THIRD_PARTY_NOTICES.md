# Third-Party Notices

## Simple Icons

PackageGet uses selected SVG brand marks from Simple Icons through the
`simple-icons-pack` Rust crate.

- Simple Icons: <https://simpleicons.org/>
- Redistribution crate: <https://github.com/2bndy5/redist-icons>
- Asset license: CC0-1.0

The marks are used only to identify the corresponding package-manager
ecosystems in the user interface. Debian, Fedora, Arch Linux, openSUSE,
Flatpak, Snapcraft, Homebrew, Rust, Go, npm, pnpm, Python, uv, .NET,
RubyGems, Composer, and NixOS names and logos may be trademarks of their
respective owners. PackageGet is not affiliated with or endorsed by those
projects.

## Lucide

The Health navigation mark is based on the Lucide Activity icon.

- Lucide: <https://lucide.dev/>
- License: ISC
